...
static class RPCEvents
{
    ...
    {
        RPCEventInvoker.ImmediatelyChangeRole(Helpers.playerById(playerId), Roles.Roles.Madmate);
    }
...
public class RPCEventInvoker
{
    ...
    public static void CreateMadmate(byte targetId, byte jailerId)
    {
        MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.CreateMadmate, Hazel.SendOption.Reliable, -1);
        writer.Write(targetId);
        writer.Write(jailerId);
        AmongUsClient.Instance.FinishRpcImmediately(writer);
        RPCEvents.CreateMadmate(targetId);
    }
